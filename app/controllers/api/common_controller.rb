...
class Api::CommonController < Api::ApplicationController
	...
	def echo
		...
		when "text"
		 	...
		 	case msg_text
		 	when "0"
				back_main_menu(user, @message, main_menu)
		 	when "1","2"
		 		unless user.nil?
		 			if msg_text == "1"
		 				show_order(user, @message)
		 			elsif msg_text == "2"
		 				begin_entry_greetings(user, @message, entry_msg)
		 			end
		 		else
		 		...
		 		if !user.nil? && user.isentry
					save_greetings(user, @message, entry_msg, msg_text)
				else
					#Keywords Match
				end
		 	end
		...
		when "event"
			...
			when "subscribe"
				@message.content = '感谢您的关注' + main_menu
			when "unsubscribe"
				@message.content = '感谢您再次关注' + main_menu
			end
			render :xml, :template => 'api/message_text'
		end
	...
	def back_main_menu(user, message, main_menu)
		user.isentry = false
		user.save
		message.save_text(main_menu)
	end

	def show_order(user, message)
		logger.debug "Query User Order.  "
	...
	def begin_entry_greetings(user, message, entry_msg)
		user.isentry = true
		user.save
		message.save_text(entry_msg)
	end

	def save_greetings(user, message, entry_msg, msg_text)
		order_no = TheBeast::Order.get_list(user.user_id)[0].order_id
		user.saveCards(order_no, message.to_user_name, msg_text, nil)
		message.save_text("保存成功！" + entry_msg)
	end
